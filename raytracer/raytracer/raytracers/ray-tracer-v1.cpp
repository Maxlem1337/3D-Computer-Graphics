...
TraceResult raytracer::raytracers::_private_::RayTracerV1::trace(const Scene& scene, const Ray& ray) const
{
	...
	if (scene.root->find_first_positive_hit(ray, &hit))
	{
		...
		MaterialProperties material_properties = hit_material->at(hit.local_position);
		
		Color hit_color = compute_ambient(material_properties);
...
Color raytracer::raytracers::_private_::RayTracerV1::compute_ambient(const MaterialProperties& material_properties) const
{
